...
export const html = (template: TemplateStringsArray, ...substitutions: (string | string[])[]): HTML => {
  ...
  const substitutionsSanitized = [
    ...substitutions.map(substitution =>
      (Array.isArray(substitution) ? substitution : [substitution]).map(substitutionPart =>
  ...
  for (const index of templatePartsSanitized.keys()) {
    ...
    else substitution = substitution.map(substitutionPart => he.encode(substitutionPart))
